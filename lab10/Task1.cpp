...
public:
	...
	bool searchname(char *name){
		for(int i=0;name[i]!='\0';i++)
			if(fName[i]!=name[i] && sName[i]!=name[i])
				return 0;
		return 1;
	}
	
	bool search(int run, float avg){
		if(runs>=run && average>=avg)
			return 1;
		return 0;
	}
...
public:
	...
	void showSelectedPlayers (int runs, float avg){
		int i;
		for(i=0;i<playersCount;i++)
			if(players[i].search(runs,avg))
				players[i].show();
	}
	...
	void sortPlayers(){
		...
		for(i=0;i<playersCount;i++)
			for(j=0;j<playersCount-1;j++)
				if(players[j].getMatchCount()<players[j+1].getMatchCount())
				{
					temp = players[j];
					players[j] = players[j+1];
					players[j+1] = temp;
				}
				else if(players[j].getMatchCount()==players[j+1].getMatchCount() && players[j].getaverage()<players[j+1].getaverage())
				{
					temp = players[j];
					players[j] = players[j+1];
					players[j+1] = temp;
...
int main(){
	...
	PlayersList list;
//	list.showAll();	
	list.searchByName("Babar");
	cout<<"-----------------------\n";
	cout << "Enter any character to proceed: ";
	cin>>temp;
	list.searchByName("Finch");
	cout<<"-----------------------\n";
	cout << "Enter any character to proceed: ";
	cin>>temp;
	list.searchByName("Anees");
	cout<<"-----------------------\n";
	cout << "Enter any character to proceed: ";
	cin>>temp;
	list.showSelectedPlayers(2500, 30);
	cout << "Enter any character to proceed: ";
	cin>>temp;
	list.sortPlayers();
	list.showAll();
